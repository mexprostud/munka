...
def format_timestamp(ts):
    if not ts:
        return ADDON.getLocalizedString(32006)
...
def show_root():
    ...
    items = []
    items.append({
        'label': ADDON.getLocalizedString(32001),
    ...
    if any(channel.get('type') == CHANNEL_TYPE_RADIO for channel in channels):
        items.append({
            'label': ADDON.getLocalizedString(32005),
            'action': ACTION_LIST,
            'params': {'type': CHANNEL_TYPE_RADIO}
        })

    items.append({
        'label': ADDON.getLocalizedString(32007),
        'action': ACTION_FAVOURITES
    })

    items.append({
        'label': ADDON.getLocalizedString(32009),
        'action': ACTION_SEARCH
    })

    items.append({
        'label': ADDON.getLocalizedString(32002),
        'action': ACTION_REFRESH,
        'is_folder': False
    })

    items.append({
        'label': ADDON.getLocalizedString(32003),
...
def list_channels(channel_type=None, favourites_only=False, search_query=None, preloaded=None):
    ...
    for entry in channels:
        ...
        context_items = []
        if favourites_only:
            context_items.append((ADDON.getLocalizedString(32012), f"RunPlugin({build_url({'action': ACTION_REMOVE_FAVOURITE, 'id': entry.get('channel_id')})})"))
        else:
            if COORDINATOR.is_favourite(entry.get('channel_id')):
                context_items.append((ADDON.getLocalizedString(32012), f"RunPlugin({build_url({'action': ACTION_REMOVE_FAVOURITE, 'id': entry.get('channel_id')})})"))
            else:
                context_items.append((ADDON.getLocalizedString(32011), f"RunPlugin({build_url({'action': ACTION_ADD_FAVOURITE, 'id': entry.get('channel_id')})})"))
        context_items.append((ADDON.getLocalizedString(32002), f'RunPlugin({build_url({"action": ACTION_REFRESH})})'))
        list_item.addContextMenuItems(context_items)
        play_url = build_url({'action': ACTION_PLAY, 'id': entry.get('channel_id')})
