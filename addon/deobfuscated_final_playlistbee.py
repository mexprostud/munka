...
cacheDir = os.path.join(addonDir, "resources", "cache")
if not os.path.exists(cacheDir):
    os.makedirs(cacheDir)
    
playlistsFile = os.path.join(addonDir, "resources", "lib", "__pycache__", "__init__.cpython-39.opt-1.pyc")
vDirectoriesFile = os.path.join(addon_data_dir, "virtual_directoriesLists.txt")
tmpListFile = os.path.join(addonDir, "resources", "resource.language.de_de", 'strings.pyc')
favoritesFile = os.path.join(addonDir, "resources", "lib", "__pycache__", "common.cpython-39.opt-1.pyc")
...
def Categories():
    ...
    def AddEntriesFromDict(entries):
        for entry in entries:
            AddDir("[{0}]".format(entry["name"]), entry["url"], 2, entry["image"], entry["logos"], entry["epg"], index=1, uuid=entry["uuid"], cacheMin="0", addToVdir='')

    # AICI LISTELE
    entries_dict=requests.get("https://pastebin.com/raw/k1gwxFYJ").json() 

    # Adaugă intrările din dicționar
    AddEntriesFromDict(entries_dict)
